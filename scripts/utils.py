...
def most_important(data, name_file):
    ...
    layout = g.layout("kk")
    fig = plot(g, layout=layout, bbox=(1000, 700))
...
def plot_graph(labels, data, color_dict, name_file, hyperparameter):
    g = Graph.TupleList([tuple(i) for i in data])
    g.vs["color"] = [color_dict[i] for i in labels]
    layout = g.layout("kk")
    fig = plot(g, layout=layout, bbox=(1000, 700))
    fig.save('../graphs/'+name_file+'_' +
             str(len(data))+'_'+hyperparameter+'.png')
    return fig
